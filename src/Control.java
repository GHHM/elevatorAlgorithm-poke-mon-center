...
public class Control {
	...
	void go(PersonInfo[] person, int currentTime, int totalTime) throws Exception {
		...
		for (Elevator e : elevatorList) {
			...
			if (e.isIdle()) {
				// if current time is between 8am and 10am and elevator is idle,
			...
			// */// 출퇴근 시간 검사 코드
			if (!e.getRequests().isEmpty()) { // check that the elevator has at
												// least one request.
				if (e.getRequests().peek() == e.getFloor()) {
					...
					e.EwaitTime++;
					if (e.EwaitTime > 2)
						e.getPassengers(person, e.getFloor(), currentTime); // the
																			// elevator
																			// is
																			// on
																			// a
			...
			} // 사람을 태우는 조건 : 그 사람이 현재보다 작은 시간에 왔고, 현재 타고 있거나 완료된 상태가 아닐경우에
				// 태운당(관련 변수는 있당)!
				// 사람을 내리는 조건 : 그 사람이 탄 엘레베이터랑 현재 엘베번호가 같고, 현재 타고 있는 상태! 완료된 상태면
				// 내리지 않음.
		}
	...
	/*
	 * method to be run when an "up" or "down" button is pressed on a floor
	...
	private Elevator getClosestEnrouteElevator(int floor, Direction direction) {
		...
		for (Elevator e : getNotEmergencyElevators()) {
			if (direction == e.getFirstDirection()) {
				...
				} else if (e.getDirection() == Direction.DOWN && direction == Direction.DOWN && e.personNum < 15
						&& e.EwaitTime < 3) {
	...
	public void EmergencyHandling(PersonInfo[] person, int pressedPersonNum) {
		...
		for (int i = 0; i < person.length; i++) {
			if (person[i].getFinished() != 1
				...
				if (person[i].getEntered() == 0 && person[i].wait == 0)// 아직
																		// 안들어갔고
																		// 버튼은
																		// 눌러진
																		// 상태
